...
void ball::reduceSpeed(float deceleration, float time) {
	// Вычисляем текущую скорость как векторную величину
	float speed = sqrt(dx * dx + dy * dy);

	// Если скорость больше нуля, уменьшаем её
	if (speed > 0) {
		// Вычисляем новое значение скорости с учётом замедления
		speed = max(0.0f, speed - deceleration * time);

		// Вычисляем коэффициенты для скорости по осям x и y
